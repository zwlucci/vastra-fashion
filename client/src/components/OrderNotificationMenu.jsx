...
export function OrderNotificationMenu() {
  ...
  return (
    <div className="relative" ref={rootRef}>
      <button className="btn-secondary relative h-10 w-10 px-0" onClick={() => setOpen((value) => !value)} type="button" title="Notifications" aria-expanded={open} aria-label="Notifications">
        <Bell size={18} />
      ...
      {open && (
        <div className="absolute right-0 top-12 z-40 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-2xl dark:border-neutral-800 dark:bg-neutral-950">
          <div className="flex items-center justify-between border-b border-neutral-200 p-3 dark:border-neutral-800"><div><p className="font-black">Updates</p><p className="text-xs text-neutral-500">Orders, returns, and vendor applications</p></div>{unreadOrderCount > 0 && <button className="text-xs font-bold text-clay hover:underline" onClick={() => markAllOrderNotificationsRead().catch(() => {})} type="button"><CheckCheck className="mr-1 inline" size={14} />Read all</button>}</div>
          <div className="max-h-96 overflow-y-auto">
            ...
            )) : <p className="p-5 text-center text-sm text-neutral-500">No updates yet.</p>}
          </div>
