        ...
        <div className="absolute right-0 top-12 z-40 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-2xl dark:border-neutral-800 dark:bg-neutral-950">
          ...
          <div className="max-h-96 overflow-y-auto">
            {notificationsLoading && !orderNotifications.length ? <p className="p-5 text-sm text-neutral-500">Loading updates...</p> : orderNotifications.length ? orderNotifications.map((item) => (
              <Link className={`block border-b border-neutral-100 p-3 last:border-0 hover:bg-neutral-50 dark:border-neutral-900 dark:hover:bg-neutral-900 ${item.read ? "" : "bg-clay/5"}`} key={item.id} to="/orders" onClick={() => { setOpen(false); if (!item.read) markOrderNotificationRead(item.id).catch(() => {}); }}>
          ...
          <Link className="block border-t border-neutral-200 p-3 text-center text-sm font-bold text-clay dark:border-neutral-800" to="/orders" onClick={() => setOpen(false)}>View all orders</Link>
        </div>
      )}
    </div>
  );
