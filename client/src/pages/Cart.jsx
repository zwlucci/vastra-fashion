...
export function Cart() {
  ...
  if (completedOrder) return <section className="mx-auto max-w-2xl px-4 py-14"><div className="panel space-y-5 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-clay/10 text-2xl text-clay">✓</div>{completedOrder.paymentMethod === "card" && <p className="text-sm font-bold uppercase tracking-wide text-clay">Payment successful</p>}<h1 className="text-3xl font-black">Order placed successfully</h1><p className="text-neutral-500">Order #{completedOrder.id.slice(0, 8)} is confirmed. A PDF receipt will be emailed to you.</p><div className="flex justify-center gap-3"><Link className="btn-secondary" to="/orders">View order</Link><Link className="btn-primary" to="/">Back to Home</Link></div></div></section>;
