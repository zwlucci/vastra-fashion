...
export function Cart() {
  ...
  async function placeOrder() {
    ...
    if (paymentMethod === "cod" && checkoutDetails.codPolicy && !checkoutDetails.codPolicy.codAvailable) { setMessage(checkoutDetails.codPolicy.warning); return; }
  ...
  return <section className="mx-auto max-w-7xl px-4 py-8"><StepHeader step={step} />{message && <p className="mx-auto mb-5 max-w-2xl rounded-md bg-red-50 p-3 text-sm text-red-700 dark:bg-red-950 dark:text-red-200">{message}</p>}
        ...
        {checkoutDetails.addresses.length > 0 && <div className="mb-5 space-y-2">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <label className={`cursor-pointer rounded-xl border p-3 text-sm ${!savedAddressId ? "border-clay bg-clay/5" : "border-neutral-200 dark:border-neutral-800"}`}>
              <span className="flex items-center gap-2 font-bold"><input checked={!savedAddressId} name="saved-address" onChange={() => setSavedAddressId("")} type="radio" />Use a new address</span>
        ...
        {savedAddressId ? (
          <div className="rounded-xl border border-clay/30 bg-clay/5 p-4 text-sm">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              ...
              <p className="sm:col-span-2"><span className="block text-xs font-bold uppercase tracking-wide text-neutral-500">Delivery address</span>{details.deliveryAddress}</p>
            </div>
          </div>
        ) : (
          <>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="text-sm font-semibold">Full name<input className="mt-1 w-full" autoComplete="name" value={details.fullName} onChange={(event) => setDetails({ ...details, fullName: event.target.value })} /></label>
              <label className="text-sm font-semibold">Phone number<input className="mt-1 w-full" autoComplete="tel" inputMode="tel" value={details.phoneNumber} onChange={(event) => setDetails({ ...details, phoneNumber: event.target.value })} /></label>
              <label className="text-sm font-semibold sm:col-span-2">Delivery location/address<textarea className="mt-1 w-full" autoComplete="street-address" rows="4" value={details.deliveryAddress} onChange={(event) => setDetails({ ...details, deliveryAddress: event.target.value })} /></label>
            </div>
            <label className="mt-4 flex cursor-pointer items-center gap-2 text-sm"><input checked={saveShippingInfo} onChange={(event) => setSaveShippingInfo(event.target.checked)} type="checkbox" /> Save this new address for future checkout</label>
          </>
        )}
    ...
    {step === 2 && <div className="grid gap-6 lg:grid-cols-[1fr_360px]"><div className="panel"><div className="mb-5 flex items-center gap-3"><CreditCard className="text-clay" /><div><p className="text-sm font-bold uppercase tracking-wide text-clay">Step 3</p><h1 className="text-3xl font-black">Payment</h1></div></div>{codPolicyPanel(checkoutDetails.codPolicy)}{checkoutDetails.paymentPreferences.length > 0 && <div className="mb-5 space-y-2"><p className="text-sm font-semibold">Saved payment preferences</p><div className="grid gap-2 sm:grid-cols-2">{checkoutDetails.paymentPreferences.map((payment) => {
