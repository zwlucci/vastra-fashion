...
export function Cart() {
  ...
  const [checkoutDetails, setCheckoutDetails] = useState({ addresses: [], paymentPreferences: [] });
  ...
  useEffect(() => {
    ...
    async function loadCheckoutDetails() {
      try {
        ...
        if (!active) return;
        setCheckoutDetails({ addresses: data.addresses || [], paymentPreferences: data.paymentPreferences || [] });
  ...
  async function placeOrder() {
    ...
    try {
      const order = await checkout({
        ...
        saveCardDetails: paymentMethod === "card" && saveCardDetails,
        savePaymentPreference: paymentMethod === "card" && saveCardDetails,
        address: { detailedAddress: details.deliveryAddress },
        ...(paymentMethod === "card" ? { card: { ...card, cardNumber: card.cardNumber.replace(/[ -]/g, "") } } : {})
      });
  ...
  return <section className="mx-auto max-w-7xl px-4 py-8"><StepHeader step={step} />{message && <p className="mx-auto mb-5 max-w-2xl rounded-md bg-red-50 p-3 text-sm text-red-700 dark:bg-red-950 dark:text-red-200">{message}</p>}
    ...
    {step === 2 && <div className="grid gap-6 lg:grid-cols-[1fr_360px]"><div className="panel"><div className="mb-5 flex items-center gap-3"><CreditCard className="text-clay" /><div><p className="text-sm font-bold uppercase tracking-wide text-clay">Step 3</p><h1 className="text-3xl font-black">Payment</h1></div></div>{checkoutDetails.paymentPreferences.length > 0 && <div className="mb-5 space-y-2"><p className="text-sm font-semibold">Saved payment preferences</p><div className="grid gap-2 sm:grid-cols-2">{checkoutDetails.paymentPreferences.map((payment) => <label className={`cursor-pointer rounded-xl border p-3 text-sm ${paymentPreferenceId === payment.id ? "border-clay bg-clay/5" : "border-neutral-200 dark:border-neutral-800"}`} key={payment.id}><span className="flex items-center gap-2 font-bold"><input checked={paymentPreferenceId === payment.id} name="payment-preference" onChange={() => { setPaymentPreferenceId(payment.id); setPaymentMethod(payment.method); }} type="radio" />{payment.displayName}{payment.isDefault && <span className="rounded-full bg-clay/10 px-2 py-0.5 text-xs text-clay">Default</span>}</span><span className="mt-1 block text-neutral-500">{payment.method === "card" ? "Saved card preference only. Enter the full test card number below." : "Ready for checkout."}</span></label>)}<label className={`cursor-pointer rounded-xl border p-3 text-sm ${!paymentPreferenceId ? "border-clay bg-clay/5" : "border-neutral-200 dark:border-neutral-800"}`}><span className="flex items-center gap-2 font-bold"><input checked={!paymentPreferenceId} name="payment-preference" onChange={() => setPaymentPreferenceId("")} type="radio" />Use another method</span></label></div></div>}<fieldset className="grid gap-3 sm:grid-cols-3"><legend className="sr-only">Payment method</legend>{[{ value: "cod", label: "Cash on Delivery" }, { value: "card", label: "Card" }, { value: "esewa", label: "eSewa", disabled: true }].map((option) => <label className={`rounded-xl border p-4 ${option.disabled ? "cursor-not-allowed opacity-50" : paymentMethod === option.value ? "border-clay bg-clay/5" : "cursor-pointer border-neutral-200 dark:border-neutral-800"}`} key={option.value}><span className="flex items-center gap-2 font-bold"><input checked={paymentMethod === option.value} disabled={option.disabled} name="payment" onChange={() => { setPaymentPreferenceId(""); setPaymentMethod(option.value); }} type="radio" />{option.label}</span>{option.disabled && <span className="mt-2 block text-xs text-clay">Coming soon</span>}</label>)}</fieldset>{paymentMethod === "cod" && <div className="mt-5 rounded-xl bg-clay/10 p-4 text-sm leading-6 text-clay"><strong>Pay when your order arrives.</strong> Please keep the exact amount ready where possible.</div>}{paymentMethod === "card" && <div className="mt-5 space-y-3 rounded-xl bg-neutral-50 p-4 dark:bg-neutral-950"><p className="flex items-center gap-2 text-xs font-semibold text-neutral-500"><LockKeyhole size={14} /> Dummy test payment. CVV and full card number are never stored.</p>{checkoutDetails.paymentPreferences.some((payment) => payment.method === "card") && <p className="text-sm text-neutral-500">Saved cards are masked preferences only. Enter the full test card number to pay.</p>}<label className="block text-sm font-semibold">Cardholder name<input className="mt-1 w-full" autoComplete="cc-name" value={card.cardholderName} onChange={(event) => setCard({ ...card, cardholderName: event.target.value })} /></label><label className="block text-sm font-semibold">Card number<input className="mt-1 w-full" autoComplete="cc-number" inputMode="numeric" maxLength="23" value={card.cardNumber} onChange={(event) => setCard({ ...card, cardNumber: event.target.value })} placeholder="4242 4242 4242 4242" /></label><div className="grid grid-cols-2 gap-3"><label className="text-sm font-semibold">Expiry<input className="mt-1 w-full" autoComplete="cc-exp" maxLength="5" value={card.expiryDate} onChange={(event) => setCard({ ...card, expiryDate: event.target.value })} placeholder="MM/YY" /></label><label className="text-sm font-semibold">CVV<input className="mt-1 w-full" autoComplete="cc-csc" inputMode="numeric" maxLength="4" type="password" value={card.cvv} onChange={(event) => setCard({ ...card, cvv: event.target.value })} /></label></div><label className="flex cursor-pointer items-center gap-2 text-sm"><input checked={saveCardDetails} onChange={(event) => setSaveCardDetails(event.target.checked)} type="checkbox" /> Save this card as a masked preference</label></div>}<div className="mt-6 flex gap-3"><button className="btn-secondary" onClick={() => setStep(1)} type="button">Back</button><button className="btn-primary" disabled={placing || !items.length || paymentMethod === "esewa"} onClick={placeOrder} type="button">{placing ? "Processing..." : paymentMethod === "card" ? "Pay Now" : "Confirm Order"}</button></div></div>{summary(null)}</div>}
  </section>;
}
