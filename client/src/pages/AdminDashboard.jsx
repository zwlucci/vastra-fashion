...
export function AdminDashboard() {
  ...
  return <section className="mx-auto max-w-7xl px-4 py-10">
    ...
    <div className="grid gap-6 lg:grid-cols-[240px_minmax(0,1fr)]">
      <aside><nav aria-label="Admin dashboard sections" className="flex gap-2 overflow-x-auto rounded-xl border border-neutral-200 bg-white p-2 dark:border-neutral-800 dark:bg-neutral-900 lg:sticky lg:top-24 lg:flex-col">{sections.map(([key, label, Icon]) => <NavLink className={({ isActive }) => `flex shrink-0 items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-bold transition ${isActive ? "bg-clay text-white" : "hover:bg-neutral-100 dark:hover:bg-neutral-800"}`} key={key} to={`/admin/dashboard/${key}`}><Icon size={17} />{label}</NavLink>)}</nav></aside>
